%% build TGexM models
%this script uses relative expression and metabolomics data from Ishii et
%al to build TGexM model
...
for mut_exp=2:8
    ...
    load('/Users/vikash/Desktop/REMI/data/iJO1366_thermo.mat') % load thermodynamic model
...
%     save(['TGexM/' exp{i} '.mat'],'model')
      ...
      %  simData/ModelsSolutions/
end

%%
exp={ 'pgm', 'pgi', 'gapC', 'zwf', 'rpe', 'wt5', 'wt7'};
for i=1:2 %1:numel(exp)
    path_save=['simData/AlternativeMCS/TGexM/Enum' exp{i} '.mat']; % once need to adjust Path 
    clear model
    load(['simData/ModelsSolutions/' exp{i} '.mat']); % this is the path of the model which one we created above
    index=[model.objIndex1B;model.metB];
%     f=zeros(numel(model.f),1);
%     f(index)=1;
%     model.f=f;
