function [model]=addRelExpCons2Models_Temp(model1,model2,regRxns,regRatio)
    ...
    objIndex2B=[];
    objBasal=[];
    objRelax=[]; % this is index for relaxing
    for i=1:numel(regRxns)
        ...
        if (Find>0) & (PFind>0) 
            ...
            bIdx1BF=length(varNames);
            % add relax variables
            varNames{length(varNames)+1,1} = ['Relax_'  regRxns{i}];
            var_ub(length(varNames),1) = bigM;
            var_lb(length(varNames),1) = 0;
            vartypes{length(varNames),1} = 'C';
            relaxC=length(varNames);

            if regRatio(i)>=1
                
                % F2 >F1*ratio-Yplus-Relax
                % F2-F1*raio+Yplus+relax>0
                [num_constr,num_vars] = size(A);
                ...
                A(num_constr+1,relaxC) = 1;% for yplus
%                 A(num_constr+1,bIdx2) = 1; % for Yconflict
            else
                % F2< F1*ratio+Yplus+Relax
                % F2-Yplus-F1*raio-Relax<0
                [num_constr,num_vars] = size(A);
                rhs(num_constr+1,1) = 0;
                constraintNames{num_constr+1,1} = ['Cplus_' 'F' regRxns{i}];
                constraintType{num_constr+1,1} = '<';
                ...
                A(num_constr+1,PFind) =1;
                A(num_constr+1,bIdx1) = -1;
                A(num_constr+1,relaxC) = -1;% for y plus
...
%             objIndex2B=[objIndex2B;bIdx2B];
        ...
        if (Rind>0) & (PRind>0) 
            ...
            vartypes{length(varNames),1} = 'B';
            bIdx1B=length(varNames);
            bIdx1BR=length(varNames);
%             % add relax variables
%             varNames{length(varNames)+1,1} = ['Relax_' 'R' regRxns{i}];
%             var_ub(length(varNames),1) = bigM;
%             var_lb(length(varNames),1) = 0;
%             vartypes{length(varNames),1} = 'C';
%             relaxC=length(varNames);
...
%             constraintNames{num_constr+1,1} = ['BinAdd' regRxns{i}];
%             constraintType{num_constr+1,1} = '=';
%             A(num_constr+1,bIdx1BR) = 1; % for YBFplus
%             A(num_constr+1,bIdx1BF) = 1; % for YBRlus
              % add relax variables
            varNames{length(varNames)+1,1} = ['RelaxBasal_' 'F1' regRxns{i}];
            var_ub(length(varNames),1) = bigM;
            var_lb(length(varNames),1) = 0;
            vartypes{length(varNames),1} = 'C';
            relaxF1=length(varNames);
            % add relax variables
            varNames{length(varNames)+1,1} = ['RelaxBasal_' 'F2' regRxns{i}];
            var_ub(length(varNames),1) = bigM;
            var_lb(length(varNames),1) = 0;
            vartypes{length(varNames),1} = 'C';
            relaxF2=length(varNames);
            
            [~,plusF]=ismember(['YBplus_' 'F' regRxns{i}],varNames);
            % F1>delta*YBplus_F-relaxF1 
            [num_constr,num_vars] = size(A);
            rhs(num_constr+1,1) = 0;
            ...
            A(num_constr+1,relaxF1) = -1;
            % F2>delta*YBplus_F-relaxF2
            [num_constr,num_vars] = size(A);
            rhs(num_constr+1,1) = 0;
            ...
            A(num_constr+1,PFind) = 1; 
            A(num_constr+1,plusF) = -basal; 
            A(num_constr+1,relaxF2) = -1;
            %
            [~,plusR]=ismember(['YBplus_' 'R' regRxns{i}],varNames);
             % add relax variables
            varNames{length(varNames)+1,1} = ['RelaxBasal_' 'R1' regRxns{i}];
            var_ub(length(varNames),1) = bigM;
            var_lb(length(varNames),1) = 0;
            vartypes{length(varNames),1} = 'C';
            relaxR1=length(varNames);
            % add relax variables
            varNames{length(varNames)+1,1} = ['RelaxBasal_' 'R2' regRxns{i}];
            var_ub(length(varNames),1) = bigM;
            var_lb(length(varNames),1) = 0;
            vartypes{length(varNames),1} = 'C';
            relaxR2=length(varNames);
            % F1>delta*YBplus_R-rleaxR1 
            [num_constr,num_vars] = size(A);
            rhs(num_constr+1,1) = 0;
            ...
            A(num_constr+1,relaxR1) = -1;
            % F2>delta*YBplus_R-relaxR2 
            [num_constr,num_vars] = size(A);
            rhs(num_constr+1,1) = 0;
            ...
            A(num_constr+1,PRind) = 1; 
            A(num_constr+1,plusR) = -basal; 
            A(num_constr+1,relaxR2) = -1;
            %
            objRelax=[objRelax;relaxF1;relaxF2;relaxR1;relaxR2];
