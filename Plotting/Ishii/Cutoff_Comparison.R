...
setwd(paste0(here(),"/Simulations/Ishii/RT_PCR_Relaxed_DFBA_Cutoff/"))
Md = read.xlsx("Results.xlsx",1, header = F)
Pd = read.xlsx("Results.xlsx",2, header = F)
Rd = read.xlsx("Results.xlsx",3, header = F)
MRatio = M/wtflux
...
rm(list= ls()[!(ls() %in% c('data', 'data2', 'conditions'))])

dat = rbind(data,data2)
dat$test = factor(dat$test, levels = c("dFBA", "REMI"))
dat$group = factor(dat$group, levels = c("NRMSE","\u03c1", "Sign Acc"))

cols = c("#FF9E4A", "#2d6d66")

setwd(paste0(here(),"/Plotting/Ishii/"))

tiff('Cutoff_Comparison.tiff', units="px", width=(700*3), height=(450*3), res=300)
