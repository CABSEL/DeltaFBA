...
M = wd[,c(9,10,15,21,24,5,6)]
rm(wd)
setwd("D:/GitHub/D_FBA/Simulations/Ishii/Array_REMI/")
...
for (j in 1:length(conditions)) {
  ...
  if (length(ind)>1) {
  Md[,j] = rowMeans(M_exp[,ind] - M_wt[,ind])
  MRatio[,j] = rowMeans(M_exp[,ind] / M_wt[,ind])
  Pd[,j] = rowMeans(P_exp[,ind] - P_wt[,ind])
  Pmut[,j] = rowMeans(P_exp[,ind])
  } else {
...
tiff('Ishii_Comparison_Array_C.tiff', units="px", width=(1200*3), height=(1000*3), res=300)
...
setwd("D:/GitHub/D_FBA/Plotting/Ishii/")
par(family = "Calibri", font = 2)
tiff('Ishii_Comparison_Array_B.tiff', units="px", width=(1000*3), height=(1000*3), res=300)
par(family = "Calibri", font = 2)
# plot with default options:
radarchart(t2 , axistype=1 , 
           #custom polygon
           ...
           #custom the grid
           cglcol="grey", cglty=1, axislabcol="black", caxislabels=seq(0,2.5,0.5), cglwd=0.8,
           #custom labels
           vlcex=1.35, calcex = 1.35
)
dev.off()
...
setwd("D:/GitHub/D_FBA/Plotting/Ishii/")
par(family = "Calibri", font = 2)
tiff('Ishii_Comparison_Array_A.tiff', units="px", width=(1000*3), height=(1000*3), res=300)
par(family = "Calibri", font = 2)
# plot with default options:
radarchart(t2 , axistype=1 , 
           #custom polygon
           pcol=colors_border ,pfcol=colors_in , plwd=4 , plty=1, seg = 2,pty = 32,
           #custom the grid
           cglcol="grey", cglty=1, axislabcol="black", caxislabels=seq(0,0.5,0.25), cglwd=0.8,
           #custom labels
           vlcex=1.35, calcex = 1.35
)
dev.off()
...
write.table(s3,"Ishii_Comparison_Array_Avg.txt", row.names = T, col.names = T, sep = "\t")