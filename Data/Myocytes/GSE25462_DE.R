...
library(GEOquery)
library(limma)
setwd("D:/GitHub/D_FBA/Data/Myocytes/")

gset <- getGEO("GSE25462", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
unique(phenodata$characteristics_ch1.3)

ngt = which(phenodata$characteristics_ch1.3==unique(phenodata$characteristics_ch1.3)[1])
t2d = which(phenodata$characteristics_ch1.3==unique(phenodata$characteristics_ch1.3)[4])
...
write.table(tT,"GSE25462_DE.txt" , row.names=F, sep="\t")
