...
for mut_exp=2:8
    ...
    % data is taken from from Elux2 paper
...
%     m_fmodel.lb(find(fmodel.f))=sol.obj_val;
    ...
    FLUX_LIST = [folderIn 'fluxmatched.txt'];
    condition = {'_rf', '_pgm', '_pgi', '_gapC', '_zwf', '_rpe', '_wt5', '_wt7'};
    MEASURED_FLUX = [folderIn 'measured_flux' condition{measured_condition} '.txt'];
...
% we already saved results in ModelsSolutions/GXFBA folder
% save('storeBIshiiNew_gxfbaMin.mat','storeB')
