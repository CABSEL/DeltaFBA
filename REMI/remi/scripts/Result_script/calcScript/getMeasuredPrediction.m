function [correl_m,m,p]=getMeasuredPrediction(eflux2_flux,FLUX_LIST,MEASURED_FLUX,measured_condition)
    %% this model is beacuse data are prepared for
    load('/Users/vikashpandey/Documents/MATLAB/EcoliRelative/GX-FBA2/Codes18Aug16/iJO1366.mat')
    fbamodel=iJO1366;
    fid = fopen(FLUX_LIST);
        ...
        while j <= length(silico_vitro)
            ...
            elseif strcmp(silico_vitro(j:(j + 4)), '_AND_')
                ...
                if strncmp('-', flux_string, 1) == 1 % flux name has a negative value in front of itself
                    ...
                    iflux = find(strcmp([ flux_string], fbamodel.rxns));
                    if ~isempty(iflux)
