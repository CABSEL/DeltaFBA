...
for i = 1:num_rev
    ...
    model.A(num_eqs+1,(ori_rxns+num_irrev+((rev_idx(i)-1)*3)+3)) = 1; %forward Z3
    model.A(num_eqs+1,(ori_rxns+num_irrev+((num_irrev+i-1)*3)+1)) = -1; % Reverse Z1
    model.A(num_eqs+1,(ori_rxns+num_irrev+((num_irrev+i-1)*3)+2)) = -1; % Reverse Z2
    model.eqtype(num_eqs+1) = '>';
    model.rhs(num_eqs+1) = 0;
    model.eqNames{(num_eqs+1),1} = strcat('Activity1_', model.varNames{rev_idx(i),1}(1:end-2));
    
    model.A(num_eqs+2,(ori_rxns+num_irrev+((rev_idx(i)-1)*3)+1)) = -1; %forward Z1
    model.A(num_eqs+2,(ori_rxns+num_irrev+((rev_idx(i)-1)*3)+2)) = -1; %forward Z2
    model.A(num_eqs+2,(ori_rxns+num_irrev+((num_irrev+i-1)*3)+3)) = 1; % Reverse Z3
    model.eqtype(num_eqs+2) = '>';
