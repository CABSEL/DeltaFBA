function [MILPstructure, model_binary] = createGeneConsistencyObj(model_binary_gene,expr_genes,de_indUP, de_indDOWN, weights, weights_option)
...
% Note that all reactions here refer to enzyme usage reactions
% Option = 1 - exclude no change reactions from Z3
% Option = 0 - Include no change reactions from Z3

% weights are for the objective function to prioritize reactions for
% regulation. 1 = Include weights in model; 0 = do not include weights in
% model.

% Note that de indices correspond to de_rxns
% Note that the no_change_idx corresponds to the irreversible model reactions

model = model_binary_gene;
...
UpGenes = (expr_genes(de_indUP));
Uweights = weights(de_indUP);
[~,upIdx] = ismember(strcat('Z1_u_',UpGenes),model.varNames);
up_weights = Uweights;
...
DownGenes = (expr_genes(de_indDOWN));
Dweights = 1./weights(de_indDOWN);
[~,downIdx] = ismember(strcat('Z2_u_',DownGenes),model.varNames);
...
% Creating an equation for maximizing Z3 of all unchanged reactions.

[~,z3idx] = ismember(strcat('Z3_u_',model.genes),model.varNames);
