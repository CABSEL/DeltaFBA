...
for j = 2:numel(conditions)
    ...
    cut_off = 1;
    [~,de_indUP]=maxk(de_exprrxns,round(cut_off*size(find(de_exprrxns>(3/2)),1)));
    [~,de_indDOWN]=mink(de_exprrxns,round(cut_off*size(find(de_exprrxns<(2/3)),1)));
    ...
    P_D(:,j-1) = Predicted;
    R_D(:,j-1) = RxnExpr_mod;
    save(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\MAT_Files\', conditions{1,j},'.mat'))
    
end

xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),M_D,1);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),P_D,2);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),R_D,3);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),Cor,4);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),intCor,5);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),NRMSE,6);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),upRecall,7);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),downRecall,8);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),Sol_stat,9);
xlswrite(strcat(root,'\Simulations\Ishii\RT_PCR_Relaxed_DFBA_Cutoff\Results.xlsx'),Sol_stat2,10);