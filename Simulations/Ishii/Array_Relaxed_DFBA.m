...
% This data is derived from DNA Array whole-genome transcriptomics
[num,txt,~] = xlsread('Data\Ishii\Transcriptomics_Array.xlsx');
genes=txt(2:end,1);
...
for j = 2:numel(conditions)
    ...
    P_D(:,j-1) = Predicted;
    R_D(:,j-1) = RxnExpr_mod;
    save(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\MAT_Files\', conditions{1,j},'.mat'))
    
end

xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),M_D,1);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),P_D,2);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),R_D,3);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),Cor,4);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),intCor,5);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),NRMSE,6);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),upRecall,7);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),downRecall,8);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),Sol_stat,9);
xlswrite(strcat(root,'\Simulations\Ishii\Array_Relaxed_DFBA\Results.xlsx'),Sol_stat2,10);