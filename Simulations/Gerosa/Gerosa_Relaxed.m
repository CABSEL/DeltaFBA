...
[num,txt,~] = xlsread('Data\Gerosa\Fluxomics.xlsx');
flux_data = num;
...
for w = 1:numel(conditions)
    ...
    for j = 1:numel(conditions)
        if w~=j
            ...
            wt=w;
            mut=j;
            [num] = readtable(strcat(root,'\Data\Gerosa\RawData\',conditions{1,mut},'-',conditions{1,wt},'.xlsx'));
            ...
            P_D(:,j) = Predicted;
            R_D(:,j) = RxnExpr_mod;
            save(strcat(root,'\Simulations\Gerosa\MAT_Files\',conditions{1,j},'vs',conditions{1,wt},'.mat'))

        end
    end
    
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),M_D,1);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),P_D,2);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),R_D,3);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),Cor,4);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),intCor,5);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),NRMSE,6);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),upRecall,7);
    xlswrite(strcat(root,'\Simulations\Gerosa\Results',num2str(w),'.xlsx'),downRecall,8);
end
xlswrite(strcat(root,'\Simulations\Gerosa\Solstats.xlsx'),sol_stat,1);
xlswrite(strcat(root,'\Simulations\Gerosa\Solstats.xlsx'),sol_stat2,2);